...
WITH latest_snapshot AS (
    ...
    WHERE trade_date = (SELECT MAX(trade_date) FROM {{ ref('prep_security_daily_momentum') }})
),
...
returns_lookback AS (
    SELECT
        ...
        return_1m,
        return_3m,
        (close - first_close_ytd) / NULLIF(first_close_ytd, 0) AS return_ytd
...
numbered_dates AS (
    ...
    WHERE trade_date >= DATEADD(
        day, -33,
        (SELECT MAX(trade_date) FROM {{ ref('prep_security_daily_momentum') }})
    )
),
