...
The Airflow DAG `market_data_pipeline` is designed to run Monday through Friday
at noon Eastern. It targets the latest completed NYSE trading date and executes:
...
`ADMIN.INGESTION_CHECKPOINTS` records each date's progression:

- `started`: ingestion began
- `archived`: the S3 object was written and verified
- `completed`: Snowflake RAW loading succeeded
- `failed`: an error occurred, with its message retained

Rows also retain source counts, inserted counts, S3 bucket and key, ETag, and
SHA-256 checksum. Completed dates are skipped by subsequent runs.

## Historical S3 Reconstruction

The retained Snowflake raw table can reconstruct the historical S3 archive
without calling the provider API. Preview the scope first:
...
Then execute the restartable backfill:
...
Optional `--start-date` and `--end-date` arguments constrain the date range.
Existing objects with matching row counts are skipped. Each newly archived
object is read back and checksum-verified. Reconstruction preserves retained
rows and operational metadata but cannot recreate the original HTTP envelope.
...
Freshness checks compare related model dates; they do not require the retained
historical snapshot to be current with today's date.
...
The raw S3 archive and `RAW.DAILY_STOCKS_RAW` intentionally coexist: S3 is the
durable replay source, while Snowflake RAW is the queryable warehouse landing
layer used by dbt.
